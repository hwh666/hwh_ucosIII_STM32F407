...
CPU_TS Cycle_T[TimeTestGroup][3];
_Time_Cost     Time_Cost;
_TimeMeasure_t TimeMeasure_t;
CPU_TS Get_Cycle_T(CPU_INT08U item)	
{
	Cycle_T[item][OLD] = Cycle_T[item][NOW];	//上一次的时间
	Cycle_T[item][NOW] = OS_TS_GET(); //本次的时间
    if(Cycle_T[item][OLD]!=0)
        Cycle_T[item][NEW] = ( ( Cycle_T[item][NOW] - Cycle_T[item][OLD] )/( get_rcc_clock.HCLK_Frequency / 1000000) );//间隔的时间（周期）
